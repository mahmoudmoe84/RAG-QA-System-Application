...
class HealthCheckResponse(BaseModel):
    """class for health check response schema."""
    status:str = Field(...,description ="service status")
    timestamp:datetime = Field(default_factory=datetime.utcnow,
                               description="response timestamp in UTC")
    #default_factory is used to generate the current UTC time when the model is instantiated
    #default_factory means that each time an instance of HealthCheckResponse is created,
    #example: HealthCheckResponse(), the timestamp field will automatically be set to the current UTC time.
...
class QueryRequest(BaseModel):
    ...
    enable_evaluation: bool = Field(default=False,description="Enable RAGAS evaluation (faithfulness, answer relevancy)",)
